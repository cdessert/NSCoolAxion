...
c in erg/cm^3/s
     ...
     &            (t/1d8)**6 * (Fxn/0.601566d0) * mstn(i)**2
...
c Brem_nn:  n+n -> n+n+2nu
     ...
     1          3d0*Cfac**2
      Fxn = funcF(xn)
      qbrem_nn= 9.18d11 * mstn(i)**4 * Fxn * (kfn(i)/1.68d0) *
     1          (t/1.d8)**8
c Brem_np:  n+p -> n+p+2nu
     ...
     1          4d0*Cfac/(1d0+4d0*xn**2) + 6d0*Cfac**2
      qbrem_np= 2.16d12 * mstn(i)**2*mstp(i)**2 * 
     1         (kfp(i)/1.68d0) * alpha_np * (t/1.d8)**8
c Brem_pp:  p+p -> p+p+2nu
      xp = 0.207d0 * (1.68d0/kfp(i))
      Fxp = funcF(xp)
      qbrem_pp= 1.14d12 * mstp(i)**4 * Fxp * 
...
c in erg/cm^3/s
     ...
     &            (t/1.d8)**6 * (Fxp/0.601556d0) * mstp(i)**2
     ...
     &            (kfp(i)/1.68d0) * (t/1.d8)**6 * mstn(i)*mstp(i)
